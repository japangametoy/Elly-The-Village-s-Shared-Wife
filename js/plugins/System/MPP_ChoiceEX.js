 ...
 * 
 * 
 *  @param Default Choice X
 *      @desc Default X coordinate for the choice window (-1: auto)
 *      @type number
 *          @min -1
 *          @max 9999
 *      @default -1
 *
 *  @param Default Choice Y
 *      @desc Default Y coordinate for the choice window (-1: auto)
 *      @type number
 *          @min -1
 *          @max 9999
 *      @default -1
 *
 *  @param Default Choice Width
 *      @desc Default width for the choice window (-1: auto)
 *      @type number
 *          @min -1
 *          @max 9999
 *      @default -1
 *
 *  @param Default Choice Height
 *      @desc Default height for the choice window (-1: auto)
 *      @type number
 *          @min -1
 *          @max 9999
 *      @default -1
 ...
 *  - 各選択肢項目の下に[注釈]で
 *      カーソル画像
 *    と入れ、続きに画像ファイル名を入れると、その選択肢にカーソルが
 *    合った時にピクチャとして表示されます。
 *  - 画像は img/pictures フォルダから読み込まれます。
 *  - ピクチャ番号やその他設定はプラグインパラメータで変更できます。
 * 
 * ================================
 ...
 * 
 *  @param Choice Cursor Image Commands
 *      @text [カーソル画像]呼び出し名
 *      @desc 
 *      @type string[]
 *      @default ["CursorImage","<CursorImage>","カーソル画像","<カーソル画像>"]
 * 
 *  @param Cursor Picture Number
 *      @text カーソル画像ピクチャ番号
 *      @desc カーソル画像を表示するピクチャ番号
 *      @type number
 *          @min 1
 *          @max 100
 *      @default 11
 * 
 *  @param Cursor Picture X
 *      @text カーソル画像X座標
 *      @desc カーソル画像のX座標
 *      @type number
 *          @min 0
...
(() => {
    ...
    const param_DefaultChoiceX = parameters['Default Choice X'] !== undefined
        ? Number(parameters['Default Choice X'])
        : -1;
    const param_DefaultChoiceY = parameters['Default Choice Y'] !== undefined
        ? Number(parameters['Default Choice Y'])
        : -1;
    const param_DefaultChoiceWidth = parameters['Default Choice Width'] !== undefined
        ? Number(parameters['Default Choice Width'])
        : -1;
    const param_DefaultChoiceHeight = parameters['Default Choice Height'] !== undefined
        ? Number(parameters['Default Choice Height'])
        : -1;
    ...
    const param_ChoiceCursorImageCommands = JSON.parse(parameters['Choice Cursor Image Commands'] || '["CursorImage","<CursorImage>","カーソル画像","<カーソル画像>"]');
    const param_CursorPictureNumber = Number(parameters['Cursor Picture Number'] || 11);
    const param_CursorPictureX = Number(parameters['Cursor Picture X'] || 0);
    const param_CursorPictureY = Number(parameters['Cursor Picture Y'] || 0);
    const param_CursorPictureOrigin = Number(parameters['Cursor Picture Origin'] || 0);
    ...
    Game_Message.prototype.clear = function () {
        ...
        this._helpTexts = [];
        this._cursorImages = [];
        this._choiceX = param_DefaultChoiceX;
        this._choiceY = param_DefaultChoiceY;
        this._choiceWidth = param_DefaultChoiceWidth;
        this._choiceHeight = param_DefaultChoiceHeight;
    ...
    Game_Message.prototype.setChoiceCursorImages = function (images) {
        this._cursorImages = images;
    };

    Game_Message.prototype.cursorImages = function () {
        return this._cursorImages;
    };
    ...
    Game_Interpreter.prototype.setupChoices = function (params) {
        ...
        if (data.choices.length > 0) {
            ...
            const cursorImages = this.choiceCursorImages(data);
            ...
            $gameMessage.setChoiceCursorImages(cursorImages);
        ...
        for (const [i, command] of this._list.slice(index).entries()) {
            if (command.indent === this._indent) {
                if (command.code === 402) {
                    ...
                    const cursorImage = this.extractCursorImage(i + index + 1);
                    if (cursorImage) data.cursorImages[type] = cursorImage;
    ...
    Game_Interpreter.prototype.extractCursorImage = function (index) {
        const command = this._list[index];
        if (
            command.code === 108 &&
            param_ChoiceCursorImageCommands.includes(command.parameters[0])
        ) {
            return this.getCursorImageName(index + 1);
        }
        return null;
    };

    Game_Interpreter.prototype.getCursorImageName = function (index) {
        const command = this._list[index];
        if (command && command.code === 408) {
            return command.parameters[0].trim();
        }
        return null;
    };

    Game_Interpreter.prototype.choiceCursorImages = function (data) {
        const cursorImages = data.cursorImages;
        return cursorImages.length > 0 ? data.results.map(i => cursorImages[i]) : [];
    };
    ...
    Window_ChoiceList.prototype.select = function (index) {
        ...
        // カーソル画像の表示処理
        if (index !== this.index()) {
            this.updateCursorImage(index);
        }
        _Window_ChoiceList_select.apply(this, arguments);
    };

    Window_ChoiceList.prototype.updateCursorImage = function (index) {
        const cursorImages = $gameMessage.cursorImages();
        if (!cursorImages || cursorImages.length === 0) return;

        const imageName = cursorImages[index];
        const pictureId = param_CursorPictureNumber;
        ...
        if (imageName) {
            ...
            $gameScreen.showPicture(
                pictureId,
                imageName,
                param_CursorPictureOrigin,
                param_CursorPictureX,
                param_CursorPictureY,
                100,  // scaleX
                100,  // scaleY
                255,  // opacity
                0     // blendMode
