 ...
 * @text コモンPadボタン0
 * @type common_event
 * @desc ゲームパッドのボタン0で起動するコモンイベント
 * 初期値: 0( 1 以上で有効) (0:A 1:B 2:X 3:Y 4:L1 5:R1)
 * @default 0
 *
 * @param commonPad1
 * @text コモンPadボタン1
 * @type common_event
 * @desc ゲームパッドのボタン1で起動するコモンイベント
 * 初期値: 0( 1 以上で有効)
 * @default 0
 *
 * @param commonPad2
 ...
 * @text コモンPadボタン12
 * @type common_event
 * @desc ゲームパッドのボタン12で起動するコモンイベント
 * 初期値: 0( 1 以上で有効) (12:上 13:下 14:左 15:右)
 * @default 0
 *
 * @param commonPad13
 * @text コモンPadボタン13
 * @type common_event
 * @desc ゲームパッドのボタン13で起動するコモンイベント
 * 初期値: 0( 1 以上で有効)
 * @default 0
 *
 * @param commonPad14
 ...
 *   マップシーンで対応するキー（またはゲームパッドボタン）を押せばコモンイベントが実行されます。
 * 
...
(function () {
  ...
  // Gamepad mapping
  for (var i = 0; i <= 15; i++) {
    var name = 'commonPad' + i;
    var commonEventId = +parameters[name];
    if (commonEventId > 0) {
      Input.gamepadMapper[i] = name;
      commonKeys[name] = commonEventId;
    }
  }
  ...
  Scene_Map.prototype.updateScene = function () {
    ...
    if (!SceneManager.isSceneChanging() && !$gameMap.isEventRunning() && !$gameMessage.isBusy() &&
