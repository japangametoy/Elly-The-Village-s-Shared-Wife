...
(() => {
  ...
  class Game_LogMessage {
    ...
    text() {
      // キャラクター名が存在する場合は四角で囲む
      const formattedName = this.speakerName
        ? `\\c[1]【${this.speakerName}】\\c[0]`  // 色コードを含む名前装飾
        : ''; // 名前が空の場合は空文字列
      ...
      return [formattedName, this.message].filter((text) => text).join('\n');
    }
    isLogSplitter() {
  ...
  class Window_TextLog extends Window_Selectable {
    ...
    processEscapeCharacter(code, textState) {
      ...
      // MessageAlignCenterプラグインの縦方向揃え制御文字（VC, VB）を無視
      // これらの制御文字はウィンドウのinnerHeightを基準に位置計算するため、
      // スクロール可能なログウィンドウでは正しく動作しない
      const upperCode = code.toUpperCase();
      if (upperCode === 'VC' || upperCode === 'VB') {
        this.obtainEscapeParam(textState); // パラメータ（[数値]）を消費
        return;
      }
    ...
    windowClass.terminateMessage = function () {
      if (!settings.disableLoggingSwitch || !$gameSwitches.value(settings.disableLoggingSwitch)) {
        if ($gameMessage.allText()) {
          ...
          text = text.replace(/\\[S][PEM]\[[^\]]*\]/gi, '');
          text = text.replace(/\\COM\[[^\]]*\]/gi, '');
          $gameTemp
            .eventTextLog()
            .pushLog(
