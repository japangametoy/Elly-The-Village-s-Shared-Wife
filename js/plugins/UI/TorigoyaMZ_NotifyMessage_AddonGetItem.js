 ...
 * @default 0
 *
 * @param baseLoseMoneyMessage
 * @text お金喪失メッセージ
 * @desc お金を失ったときのメッセージを設定します。（\gold : 喪失金額）
 * @parent base
 * @type string
 * @default \gold\c[4]\G\c[0] を失った……
 *
 * @param baseLoseMoneyIcon
 * @text お金喪失アイコン
 * @desc お金を失ったときのアイコンIDを設定します。0の場合はアイコンを表示しません。
 * @parent base
 * @type number
 * @decimals 0
 * @default 0
 ...
 * @default {"overwrite":"false","sound":"{\"name\":\"\",\"volume\":\"90\",\"pitch\":\"100\",\"pan\":\"0\"}"}
 *
 * @param advancedLoseMoneySound
 * @text お金喪失効果音の上書き
 * @desc お金を失ったときに再生する効果音を指定します。
 * 上書き設定をしない場合は通常の通知音が再生されます。
 * @parent advanced
 * @type struct<CustomSound>
 * @default {"overwrite":"false","sound":"{\"name\":\"\",\"volume\":\"90\",\"pitch\":\"100\",\"pan\":\"0\"}"}
...
(function () {
    ...
    function readParameter() {
        ...
        return {
            ...
            baseLoseMoneyMessage: parseStringParam(
                parameters['baseLoseMoneyMessage'],
                '\\gold\\c[4]\\G\\c[0] Lost……',
            ),
            baseLoseMoneyIcon: parseIntegerParam(parameters['baseLoseMoneyIcon'], 0),
            ...
            advancedLoseMoneySound: readStructCustomSound(
                parseStructObjectParam(parameters['advancedLoseMoneySound'], {
                    overwrite: false,
                    sound: { name: '', volume: 90, pitch: 100, pan: 0 },
                }),
            ),
    ...
    (() => {
        ...
        Torigoya.NotifyMessage.Addons.GetItem.notifyGainMoney = function (value) {
            ...
            if (value > 0) {
                ...
                const template = Torigoya.NotifyMessage.Addons.GetItem.parameter.baseLoseMoneyMessage;
                if (!template) return;

                const notifyItem = new Torigoya.NotifyMessage.NotifyItem({
                    message: template.replace(/\\gold/, Math.abs(value)),
                    icon: Torigoya.NotifyMessage.Addons.GetItem.parameter.baseLoseMoneyIcon,
                    note: '<type:loseMoney>',
                });
                Torigoya.NotifyMessage.Manager.notify(notifyItem);
            }
        ...
        Torigoya.NotifyMessage.NotifyItem.prototype.getDisplaySe = function () {
            switch (this.meta.type) {
                ...
                case 'loseMoney': {
                    if (Torigoya.NotifyMessage.Addons.GetItem.parameter.advancedLoseMoneySound.overwrite) {
                        return Torigoya.NotifyMessage.Addons.GetItem.parameter.advancedLoseMoneySound.sound;
                    }
                    break;
                }
            }
        ...
        // -------------------------------------------------------------------------
        // Game_Party - スクリプトからのgainGold呼び出しにも対応

        // イベントコマンドからの呼び出し時に二重通知を防ぐためのフラグ
        Torigoya.NotifyMessage.Addons.GetItem._skipGainGoldNotify = false;

        const upstream_Game_Party_gainGold = Game_Party.prototype.gainGold;
        Game_Party.prototype.gainGold = function (amount) {
            // スクリプトからの呼び出し時に通知を表示
            if (!Torigoya.NotifyMessage.Addons.GetItem._skipGainGoldNotify && amount !== 0) {
                Torigoya.NotifyMessage.Addons.GetItem.notifyGainMoney(amount);
            }
            return upstream_Game_Party_gainGold.apply(this, arguments);
        };
        ...
        Game_Interpreter.prototype.command125 = function (params) {
            ...
            // イベントコマンドからの呼び出しなので、gainGold側での通知をスキップ
            if (value !== 0) {
                Torigoya.NotifyMessage.Addons.GetItem._skipGainGoldNotify = true;
                Torigoya.NotifyMessage.Addons.GetItem.notifyGainMoney(value);
            }
            const result = upstream_Game_Interpreter_command125.apply(this, arguments);
            Torigoya.NotifyMessage.Addons.GetItem._skipGainGoldNotify = false;
            return result;
        };
