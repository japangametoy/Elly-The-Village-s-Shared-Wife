 ...
 * 
 * @command ignore
 * @text [高度]ignore/一時無効化
 * @desc 指定した名前のキャラの自動表示を一時的に無効化
 *
 * @arg name
 * @text 対象キャラ名
 * @desc 自動表示を無効化する名前（カンマ区切りで複数指定可）
 * 
 * @command unignore
 * @text [高度]unignore/無効化解除
 * @desc 指定した名前のキャラの自動表示の無効化を解除
 *
 * @arg name
 * @text 対象キャラ名
 * @desc 無効化を解除する名前（カンマ区切りで複数指定可、allで全解除）
 * 
 */
//=============================================================================
...
(function () {
	...
	//=============================================================================
	(() => {
		const commands = ['start', 'end', 'preload', 'clear', 'show', 'hide', 'fadeIn', 'fadeOut', 'slideIn', 'slideOut', 'moveIn', 'moveOut', 'expression', 'pose', 'move', 'yMove', 'jump', 'step', 'shake', 'rotate', 'flip', 'autoFocus', 'focus', 'unfocus', 'scale', 'opacity', 'tint', 'emphasize', 'animation', 'front', 'wait', 'auto', 'sequence', 'loop', 'mob', 'name', 'ignore', 'unignore'];
	...
	Skit.prototype._processCommand = function (args, macroPos) {
		...
		switch (skitCommand) {
			...
			case '無効化':
			case '無視':
				this.ignoreSkitActor(args, macroPos);
				break;
			case 'unignore':
			case '無効化解除':
			case '無視解除':
				this.unignoreSkitActor(args, macroPos);
				break;
			case 'eval':
			...
			case '一時退場':
			case '退避':
				this.evacuateSkitActors(args, macroPos);
				break;
			case 'restore':
			case '復帰':
			case '再登場':
				this.restoreSkitActors(args, macroPos);
				break;
	...
	Skit.prototype.showSkitActor = function (args, macroPos) {
		...
		var evacuated = this.peekEvacuatedData(name);
		if (evacuated && position === undefined) {
	 ...
	 *  [1] : names
	===================================*/
	Skit.prototype.ignoreSkitActor = function (args, macroPos) {
		var names = args[1].split(',');
		for (var name of names) {
			name = name.trim();
			// resolve to internal name if possible
			var inputList = parameters.nameToInputList;
			if (inputList && inputList[name]) {
				name = inputList[name];
			}
	 ...
	 *  [1] : names (or 'all')
	===================================*/
	Skit.prototype.unignoreSkitActor = function (args, macroPos) {
		var nameArg = args[1];
		if (nameArg === 'all' || nameArg === '全員' || nameArg === '全て') {
			this.clearIgnoreNames();
			return;
		}
	...
	/* clearActors
	===================================*/
	Skit.prototype.processClearActors = function (wait, keepShowingState) {
		var actors = this.actors();
		...
		if (keepShowingState) {
			...
			for (var i = 0; i < length; i = (i + 1) | 0) {
				var actor = actors[i];
				if (actor.isShowing()) {
					this._evacuatedActors[actor._name] = {
						position: actor._position,
						pose: actor._pose,
						expression: actor._expression,
						scaleX: actor._scaleX,
						scaleY: actor._scaleY,
						angle: actor._angle,
						z: actor._z,
						opacity: actor._opacity
					};
	...
	Skit.prototype.peekEvacuatedData = function (name) {
		return this._evacuatedActors && this._evacuatedActors[name];
	};
	Skit.prototype.consumeEvacuatedData = function (name) {
		if (this._evacuatedActors && this._evacuatedActors[name]) {
			var data = this._evacuatedActors[name];
			delete this._evacuatedActors[name];
			return data;
		}
		return null;
	};

	Skit.prototype.evacuateSkitActors = function (args, macroPos) {
		var wait = TRP_CORE.supplementSkitWait(parameters.defaultDisappear.wait, args[1]);
		this.processClearActors(wait, true);
	};

	Skit.prototype.restoreSkitActors = function (args, macroPos) {
		if (!this._evacuatedActors) return;
		var names = Object.keys(this._evacuatedActors);
		if (names.length === 0) return;

		var fadeInParams = parameters.fadeIn;
		var duration = fadeInParams.duration;
		var easeType = fadeInParams.easeType;
		var pushIn = fadeInParams.pushIn;
		var defaultWait = fadeInParams.wait;

		var waitArg = args[1];
		var wait = TRP_CORE.supplementSkitWait(defaultWait, waitArg);
		...
		for (var i = 0; i < length; i = (i + 1) | 0) {
			...
			actor.fadeIn(wait, easeType, data.position, duration, undefined);
	...
	/* ignore
	===================================*/
	Skit.prototype.ignoreName = function (name) {
		this._ignoreNames = this._ignoreNames || [];
		if (!this._ignoreNames.contains(name)) {
			this._ignoreNames.push(name);
		}
	};
	Skit.prototype.unignoreName = function (name) {
		this._ignoreNames = this._ignoreNames || [];
	...
	Skit.prototype.clearIgnoreNames = function () {
		this._ignoreNames = [];
	};
	Skit.prototype.isIgnored = function (name) {
		return this._ignoreNames && this._ignoreNames.contains(name);
	};
	...
	Skit.prototype.autoShowSkitActor = function (targetActor) {
		...
		if (actorLen === 0 || actorLen === 1 && actors[0] === targetActor) {
			...
			var evacuated = this.peekEvacuatedData(targetActor._name);
			if (evacuated) {
				position = evacuated.position;
			}
	...
	SkitActor.prototype.show = function (position, opacity) {
		...
		var evacuated = $gameSkit.consumeEvacuatedData(this._name);
		if (evacuated) {
			this._scaleX = evacuated.scaleX;
			this._scaleY = evacuated.scaleY;
			this._angle = evacuated.angle;
			this._z = evacuated.z;
			this._pose = evacuated.pose;
			this._expression = evacuated.expression;

			//apply restored params
			this.apply(true);
		} else {
			this.apply(true);
		}
	...
	Game_Message.prototype.add = function (text) {
		...
		if (!$gameMessage.isBusy() && $gameSkit.isSkitOn()) {
			...
			if (this._talkingActorName) {
				var isIgnored = $gameSkit.isIgnored(this._talkingActorName);
				if (!isIgnored) {
