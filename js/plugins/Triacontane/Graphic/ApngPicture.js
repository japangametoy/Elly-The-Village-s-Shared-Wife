...
// RPGツクールMZの暗号化キーを取得
const encryptionKey = $dataSystem ? $dataSystem.encryptionKey : null;

// 暗号化キーが存在しない場合のエラー処理
if (!encryptionKey) {
    console.error("暗号化キーが取得できません。System.jsonを確認してください。");
}
...
static decryptResource(key) {
    ...
    if (!resource) {
        console.error(`リソースが見つかりません: ${key}`);
        return;
    }
    try {
        // 暗号化キーを使用してデータを復号化
        resource.data = Utils.decryptArrayBuffer(resource.data, encryptionKey);
        ...
        console.log(`リソースを復号化しました: ${key}`);
    } catch (error) {
        console.error(`リソースの復号化に失敗しました: ${key}, エラー内容: ${error.message}`);
    }
}
;
