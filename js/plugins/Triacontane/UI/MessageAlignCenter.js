 ...
 * \vb : 縦方向の下揃え
 *
 * 後ろに数値を指定すると描画位置が指定したピクセル分、左に(縦揃えの場合は上に)ずれて描画されます。
 * \ac[20]
 * \vb[20]
 *
...
(() => {
        ...
        if (textState.drawing) {
            ...
            switch (code) {
                case dummy.findEscapeCenter():
                    ...
                    return; // カスタム制御文字の処理後はreturnしてオリジナル関数を呼び出さない
    ...
    class Window_MessageDummy extends Window_Message {
        ...
        findInnerVSpace(textState, innerHeight) {
            const shift = this.obtainEscapeParam(textState);
            return innerHeight - this.textSizeEx(textState.text).height - textState.startY - (shift || 0);
        }
