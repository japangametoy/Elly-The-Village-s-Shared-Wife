...
(() => {
    ...
    class Window_CustomMenu extends Window_StatusBase {
        ...
        update() {
            ...
            this.updateOkRelease();
        }

        updateOkRelease() {
            if (this._waitOkRelease && !Input.isPressed('ok') && !TouchInput.isPressed()) {
                this._waitOkRelease = false;
            }
        }
        ...
        drawItem(index) {
            ...
            const isDisabledBySwitch = !enabled && this.isSwitchInvalid && item && item.DisableSwitchId && this.isSwitchInvalid(item.DisableSwitchId);
            ...
            if (isDisabledBySwitch) {
                ...
                this.contents.fillRect(rect.x, rect.y, rect.width, rect.height, 'rgba(0, 0, 0, 0.4)');
                context.restore();
            }
        }
        ...
        activate() {
            ...
            if (Input.isPressed('ok')) {
                this._waitOkRelease = true;
            }
        }

        processOk() {
            if (this._waitOkRelease) {
                return;
            }
            super.processOk();
        }
    ...
    class Window_CustomMenuCommand extends Window_CustomMenu {
        ...
        isEnabledSub(item) {
            return item && this.isScriptValid(item.IsEnableScript) &&
                this.isSwitchValid(item.EnableSwitchId) && !this.isSwitchInvalid(item.DisableSwitchId);
        }
        ...
        findDecisionEvent() {
            const item = this.getItem();
            const isCancel = item?.CancelChoice && String(item.CancelChoice) !== 'false';
            if (isCancel) {
